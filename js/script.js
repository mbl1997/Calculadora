document.addEventListener("DOMContentLoaded", function () {
    ...
    var listenerBtn = []; //Array de elementos que receber uma listener
    ...
    function passarValorTela() {
        ...
        if (verificarOperador(this.value)) {
            var aux = tela.value.substring(tela.value.length - 1, tela.value.length); //subtituir o valor do operador pelo atual
            if (verificarOperador(aux)) {
                apagarAnterior();
            }
        }
